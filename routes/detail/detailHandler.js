...
module.exports.updatedetail = async (req, res, next) => {
  ...
  const data = await detailsService.updatedetail(
    req.params.userid,
    req.body,
    req.ip,
    req.useragent
...
module.exports.deletedetail = async (req, res, next) => {
  ...
  const data = await detailsService.deletedetail(
    req.params.userid,
    req.ip,
    req.useragent
  );
  logger.data("result ", data);
  return responser.send(200, "global", "G_E014", req, res, {
...
module.exports.getdetail = async (req, res, next) => {
  ...
  const data = await detailsService.getdetail(
    req.params.userid,
    req.ip,
    req.useragent
  );
